...
def test_build_settings_prefers_env_over_dotenv_and_yaml(tmp_path: Path) -> None:
    ...
    assert settings.redis_namespace == "smartcloud:test:business-tools"
    ...
    assert settings.runtime_data_dir == str(tmp_path / ".tmp" / "business-tools-service")
...
def test_build_settings_derives_degraded_runtime_paths_for_redis_backends(tmp_path: Path) -> None:
    config_dir = tmp_path / "config" / "settings"
    config_dir.mkdir(parents=True)
    (config_dir / "dev.yaml").write_text("", encoding="utf-8")
    ...
    runtime_dir = tmp_path / ".tmp" / "business-tools-service"
    assert settings.runtime_data_dir == str(runtime_dir)
    assert settings.idempotency_store_path == str(runtime_dir / "degraded-idempotency-store.json")
