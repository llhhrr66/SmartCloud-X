...
class Settings(BaseModel):
    ...
    auth_validation_mode: Literal["local", "strict"] = Field(
        default_factory=lambda: os.getenv("RESEARCH_SERVICE_AUTH_VALIDATION_MODE", "local")
    )
    auth_validate_token_url: str | None = Field(
        default_factory=lambda: _optional_env("RESEARCH_SERVICE_AUTH_VALIDATE_TOKEN_URL")
    )
    auth_validate_timeout_seconds: float = Field(
        default_factory=lambda: float(os.getenv("RESEARCH_SERVICE_AUTH_VALIDATE_TIMEOUT_SECONDS", "3"))
    )
    ...
    default_estimated_minutes: int = Field(
        default_factory=lambda: int(os.getenv("RESEARCH_SERVICE_ESTIMATED_MINUTES", "5"))
    )
