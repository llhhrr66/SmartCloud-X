  ...
  switch (event.event) {
    ...
    case 'faq':
      return {
        ...previous,
        reconnecting: false,
        documentRefs: event.data.documentRefs ?? [],
        lastEventAt: receivedAt
      };
    case 'done':
      return {
...
// --- Combined ChatStore (zustand-compatible selector API for chat/ChatPage) ---
...
export interface ChatStoreSnapshot {
  ...
  setMessages: (conversationId: string, items: ChatMessage[]) => void;
  appendMessage: (conversationId: string, message: ChatMessage) => void;
  upsertConversation: (item: ConversationSummary) => void;
  removeConversation: (conversationId: string) => void;
  ...
  finishStreaming: (reason: string, error?: string) => void;
}

const chatActions = {
  setConversations: conversationStoreActions.setSessions,
  setMessages: messageStoreActions.setConversationMessages,
  appendMessage: messageStoreActions.appendMessage,
  upsertConversation: conversationStoreActions.upsertConversation,
  removeConversation: conversationStoreActions.removeConversation,
  selectConversation: () => {},
  beginStreaming: (conversationId: string, messageId: string) => sseStoreActions.start(conversationId, messageId),
  applyStreamEvent: sseStoreActions.applyEvent,
  finishStreaming: (reason: string, error?: string) => {
    sseStoreActions.updateStream((prev) => ({
      ...prev,
      isStreaming: false,
...
} satisfies Omit<ChatStoreSnapshot, 'conversations' | 'messagesByConversation' | 'streaming'>;

let _chatSnapshot: ChatStoreSnapshot;

function buildChatSnapshot(): ChatStoreSnapshot {
  const conv = conversationStore.getSnapshot();
  const msg = messageStore.getSnapshot();
  const sse = sseStore.getSnapshot();
  const lastReasoning = sse.stream.reasoning.at(-1);

  return {
    conversations: conv.items,
    messagesByConversation: msg.byConversationId,
    streaming: {
      status: sse.stream.error ? 'error' : sse.stream.isStreaming ? 'running' : 'idle',
      content: sse.stream.partialContent,
      agent: sse.stream.agent,
      reasoning: lastReasoning?.summary,
      documentRefs: sse.stream.documentRefs
    },
    ...chatActions
  };
}

function getChatSnapshot(): ChatStoreSnapshot {
  const next = buildChatSnapshot();
  if (
    _chatSnapshot &&
    _chatSnapshot.conversations === next.conversations &&
    _chatSnapshot.messagesByConversation === next.messagesByConversation &&
    _chatSnapshot.streaming.status === next.streaming.status &&
    _chatSnapshot.streaming.content === next.streaming.content &&
    _chatSnapshot.streaming.agent === next.streaming.agent &&
    _chatSnapshot.streaming.reasoning === next.streaming.reasoning &&
    _chatSnapshot.streaming.documentRefs === next.streaming.documentRefs
  ) {
...
function subscribeChat(listener: () => void): () => void {
  const u1 = conversationStore.subscribe(listener);
  const u2 = messageStore.subscribe(listener);
  const u3 = sseStore.subscribe(listener);
  return () => { u1(); u2(); u3(); };
}

export function useChatStore(): ChatStoreSnapshot;
export function useChatStore<T>(selector: (s: ChatStoreSnapshot) => T): T;
export function useChatStore<T>(selector?: (s: ChatStoreSnapshot) => T): T | ChatStoreSnapshot {
  const state = useSyncExternalStore(subscribeChat, getChatSnapshot, getChatSnapshot);
  return selector ? selector(state) : state;
}

useChatStore.getState = getChatSnapshot;
