...
- frozen orchestrator streaming contracts now cover the current SSE event payloads and emitted `meta -> reasoning -> retrieval -> tool_call -> tool_result -> delta -> citation -> done` sequence used by the live orchestrator baseline, plus stored message-event replay/list baselines for `GET /api/v1/chat/sessions/{conversation_id}/messages/{message_id}/events*`
...
- frozen auth-continuation contracts now also cover additive `user_profile_bindings` on tool/user-action hint payloads plus additive orchestrator `user_profile_patch` continuation input and the documented runtime `session_context.attributes.auth_profile` persistence convenience layer
...
- frozen orchestrator, tool-hub, and business-tools ops contracts now distinguish degraded runtime health from strict readiness by documenting additive `/healthz` runtime snapshots plus `GET /readyz` readiness payloads and nested downstream `dependencyReadiness` metadata
...
- shared `TraceContext` now treats omitted and explicit `null` optional scope members as equivalent unknown-context states, matching the live knowledge-service bootstrap/admin responses
...
- shared runtime config now also reserves `SMARTCLOUD_MYSQL_DSN`, `SMARTCLOUD_REDIS_URL`, and shared `SMARTCLOUD_MINIO_*` raw-object connector keys, while `docs/contracts/shared/persistence-backends.md` records the current authoritative-backend matrix and local-fallback policy
...
- foundation is intentionally not creating `packages/common-schemas/frontend/**` yet; frontend-sdk may keep owned user-business typing outlets until the backing billing/order/refund/ticket/ICP/file/citation-detail service contracts are promoted through frozen OpenAPI/common-schema baselines, and that defer still applies to the SDK-owned `GET /api/v1/icp/applications` list contract
...
- the error catalog lives at `packages/common-schemas/errors/error_codes.yaml` and now includes `ORCH_SESSION_STATE_NOT_FOUND`, `ORCH_TOOL_CALL_NOT_FOUND`, `IDEMPOTENCY_CONFLICT`, and `CHAT_STREAM_EVENTS_NOT_FOUND` for the published state, audit, replay-conflict, and stream-replay baselines
- `@smartcloud-x/common-schemas` now keeps the exported `FoundationErrorCode` / `foundationErrorCodes` TypeScript surface aligned with the full frozen YAML error catalog so downstream frontend/backend consumers do not need owned supplement lists for already-promoted codes
...
- shared docs now include `docs/contracts/shared/runtime-health.md` for canonical backend-evidence field names and `docs/contracts/shared/persistence-backends.md` for the current service-by-service authoritative backend matrix used by QA and infra migration work; together they define the shared persistence/backend matrix for frozen-space migration decisions
...
- frozen orchestrator OpenAPI now also tracks the live process-local admin agent-config routes `GET /api/v1/admin/agents` and `PATCH /api/v1/admin/agents/{agent_code}` together with shared admin agent record/list/update DTOs and documented owner-local runtime keys `DEFAULT_AGENT_TIMEOUT_SECONDS` / `AGENT_CONFIG_STORE_PATH`
