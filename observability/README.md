...
The compose smoke test now asserts that key knowledge and RAG metrics are actually exposed on `/metrics`, and it also verifies snapshot/profile consistency plus the empty-result answer path, which keeps the observability placeholder baseline honest even before Alertmanager, OTLP spans, or LangSmith-backed evaluations are fully wired.

The backend services now also emit OTLP spans to Phoenix-compatible collectors for request handling plus key child operations such as ingestion, filesystem import, snapshot export, indexing-worker connector processing, retrieval candidate search, upstream knowledge-service fetches, and answer composition. Health and metrics routes are excluded from tracing on purpose so the Phoenix view stays focused on operator and retrieval flows instead of scrape noise.

When operators need a stronger tracing check than “the env vars are present,” run `python3 deploy/docker-compose/trace-smoke.py`. It launches temporary local knowledge-service, knowledge-indexer, and rag-service processes against a throwaway OTLP collector, exercises ingestion plus snapshot export plus worker processing plus answer traffic, and fails fast if OTLP batches are missing, either backend service is absent from the exported spans, the snapshot/worker spans are missing, or the rag-to-knowledge handoff does not preserve a shared trace ID.
