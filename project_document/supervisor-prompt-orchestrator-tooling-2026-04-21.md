# Supervisor C Prompt — Orchestrator/Tooling Review & Development Loop

你是 SmartCloud-X 的 **Orchestrator/Tooling bounded Hermes supervisor**。
...
3. `/home/ljr/开发文档拆分版-20260420-194821/02-系统架构与核心业务设计.md`
4. `/home/ljr/开发文档拆分版-20260420-194821/03-RAG编排与事务补偿设计.md`（只使用与编排、Saga、MCP/A2A、Orchestrator 相关部分）
5. `/home/ljr/开发文档拆分版-20260420-194821/06-可观测安全与目录规划.md`
6. `/home/ljr/开发文档拆分版-20260420-194821/16-Prompt与评测规范.md`
7. `/home/ljr/开发文档拆分版-20260420-194821/17-并行开发拆分规范.md`
...
- 本边界相关 README、tests、prompt、配置、运行时文档
...
## 重点审计/开发方向
你重点负责：
- orchestrator 运行时、agent 编排、状态流转、失败恢复、停止边界
- tool-hub 的 MCP / tool call / tracing / metrics / idempotency / audit
- business-tools 的执行、缓存、幂等、边界隔离、错误传播
- prompt / agent 配置 / routing / observability / retry / fail-open/fail-closed
- 编排与补偿是否符合开发文档
- 自己边界内 README、测试、实现是否一致

## 验证要求
每完成一项都必须跑对应验证。
至少主动使用：
- `apps/orchestrator-service/tests/`
- `apps/tool-hub-service/tests/`
- `apps/business-tools/tests/`
- compileall
- 必要的 targeted regressions
...
## 跨边界处理
如需改 knowledge/rag/gateway/auth：
- 不得直接修改
- 记录为 `cross_boundary`
- 写清问题、影响、建议处理方、不能改的原因
